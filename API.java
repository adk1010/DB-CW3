...
public class API implements APIProvider {
    ...
    * API constructor
    *
    * @param c The connection to the database (ensure Foreign Key Constraints
    * are enabled)
    */
   ...
   /**
    * <p>
    * Private method for unit testing API.java</p>
    * <p>
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>How it works:</b> makes call to test() which checks return state equals
    * expected return state. Prints number of tests failed and number of tests
    * passed.
    * </p>
    */
   private void tests() {
   ...
   /**
    * <p>
    * Wrapper for non-verbose printing to console</p>
    * <p>
    * <b>Main Contributor:</b> Tom</p>
    */
   ...
   /**
    * <p>
    * Get the detailed view of a single forum.</p>
    * <p>
    * <b>Visual Test:</b> </p>
    * <p>
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL:</b> SELECT id as topicid, title as topictitle FROM Topic WHERE
    * forumid = ?;</p>
    * <p>
    * <b>How it works:</b>
    *
   ...
   private boolean test(Result r, String expectedResult) {
      try {
         switch (expectedResult) {
            ...
            case "failure":
               if (!r.isFatal()) {
                  return true; //isFatal returns false if is failiure, exception if success, true if fatal
               }
               return false;
            case "fatal":
               if (r.isFatal()) {
   ...
   /**
    * <p>
    * Get a list of all users in the system as a map username -> name.</p>
    * <p>
    * <b>Visual Test:</b> http://localhost:8000/people</p>
    * <p>
    * <b>Main Contributor:</b> Joseph</p>
    * <p>
    * <b>SQL:</b> SELECT username, name FROM Person;</p>
    * <p>
    * <b>How it works:</b>
    *
    *
    * </p>
    *
    * @return A map with one entry per user of the form username -> name (note
   ...
   /**
    * <p>
    * Get a PersonView for the person with the given username.</p>
    * <p>
    * <b>Visual Test:</b> http://localhost:8000/person/tb15269</p>
    * <p>
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL:</b> SELECT name, username, stuId FROM Person " + "WHERE username =
    * ?;</p>
    * <p>
    * <b>How it works:</b>
    *
    ...
    * title. Simple version that does not return any topic information.</p>
    * <p>
    * <b>Visual Test:</b> http://localhost:8000/forums0</p>
    * <p>
    * <b>Main Contributor:</b> Alex</p>
    * <p>
    * <b>SQL:</b> SELECT id, title FROM Forum;</p>
    * <p>
    * <b>How it works:</b>
    *
    *
   ...
   /**
    * <p>
    * Count the number of posts in a topic (without fetching them all).</p>
    * <p>
    * <b>Visual Test:</b> </p>
    * <p>
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL:</b> SELECT COUNT(*) AS numposts FROM POST WHERE topicid = ?;</p>
    * <p>
    * <b>How it works:</b>
    *
    *
    ...
    * <b>Main Contributor:</b> Joseph</p>
    * <p>
    * <b>SQL:</b> SELECT Topic.id, Person.name, Person.username, Person.stuId
    * FROM Topic LEFT OUTER JOIN Topic_Likers ON topic.id = Topic_Likers.topicid
    * LEFT OUTER JOIN Person ON Topic_Likers.personid = Person.id WHERE Topic.id
    ...
    * @return Success (even if the list is empty) if the topic exists, failure
    * if it does not, fatal in case of database errors.
    */
   @Override
   ...
   /**
    * <p>
    * Get a simplified view of a topic.</p>
    * <p>
    * <b>Visual Test:</b> http://localhost:8000/topic0/1</p>
    * <p>
    * <b>Main Contributor:</b> Alex</p>
    * <p>
    * <b>SQL:</b> SELECT t.id as topicid, t.title, p.id as postid, per.username,
    * p.text, p.postedAt FROM Topic AS t JOIN Post AS p ON t.id = p.topicid JOIN
    * Person AS per ON p.authorid = per.id WHERE t.id = 1;</p>
    * <p>
    * <b>How it works:</b>
    *
    *
    ...
    * @return The topic view if one exists with the given id, otherwise failure
    * or fatal on database errors.
    */
   @Override
    ...
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL:</b> SELECT forum.id as forumid, post.topicid as topicid, post.id
    * as postNumber, person.name as authorname, person.username as username,
    * post.text as text, post.postedAt as postedAt, likes.numLikes as
    * numberOfLikes FROM Post JOIN Person ON Post.authorid = Person.id JOIN
    * Topic ON Post.topicid = Topic.id JOIN Forum ON Topic.forumid = Forum.id
    * JOIN (SELECT postid, count(*) as numLikes FROM Post_Likers GROUP BY
    * postid) as Likes ON Likes.postid = Post.id WHERE topicid = ? ORDER BY
    * postNumber DESC LIMIT 1;</p>
    * <p>
    * <b>How it works:</b>
    *
    *
    ...
    * @return Success and a view of the latest post if one exists, failure if
    * the topic does not exist, fatal on database errors.
    */
   @Override
...
http://localhost:8000/forums
    ...
    * title.</p>
    * <p>
    * <b>Visual Test:</b> http://localhost:8000/forums</p>
    * <p>
    * <b>Main Contributor:</b> Alex</p>
    * <p>
    * <b>SQL:</b> SELECT lastTopic.id AS topicid, lastTopic.title AS topicTitle,
    * f.title AS title, f.id AS id FROM Forum AS f JOIN ( SELECT t.id AS id,
    * t.forumid AS forumid, t.title AS title, p.postedAt AS postedAt FROM Topic
    * t JOIN Post AS p ON t.id = p.topicid GROUP BY p.postedAt ) AS lastTopic ON
    * f.id = lastTopic.forumid GROUP BY f.title;</p>
    * <p>
    * <b>How it works:</b>
    *
    *
   ...
   /**
    * <p>
    * Create a new forum.</p>
    * <p>
    * <b>Visual Test:</b> http://localhost:8000/newforum</p>
    * <p>
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL:</b> INSERT INTO Forum (title) VALUES (?);</p>
    * <p>
    * <b>How it works:</b>
    *
    *
    * </p>
    *
    * @param title the title of the forum. Must not be null or empty and no
    * forum with this name must exist yet.
    * @return success if the forum was created, failure if the title was null,
    * empty or such a forum already existed; fatal on other errors.
    */
   ...
   public Result createForum(String title) {
      try (PreparedStatement createStatement = c.prepareStatement(
         ...
         if (title == null) {
            return Result.failure("Cannot have forum with null title");
         }
         if (title.isEmpty()) {
            return Result.failure("Cannot have forum with no title");
         }
    ...
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL:</b> DELETE FROM Forum WHERE title = ?;</p>
    * <p>
    ...
    * <p>
    * Create a post in an existing topic.</p>
    * <p>
    * <b>Visual Test:</b> http://localhost:8000/newpost/1</p>
    * <p>
    * <b>Main Contributor:</b> Alex</p>
    * <p>
    * <b>SQL:</b> INSERT INTO Post (authorid, topicid, text) VALUES ((SELECT id
    * FROM Person WHERE username = ?), ?, ?);</p>
    * <p>
    * <b>How it works:</b>
    *
    *
    * </p>
    *
    * @param topicId the id of the topic to post in. Must refer to an existing
    * topic.
   ...
   public Result createPost(long topicId, String username, String text) {
      try (PreparedStatement createStatement = c.prepareStatement(
         ...
         if (ex.getLocalizedMessage().contains("FOREIGN KEY constraint failed")) {
            ...
            return Result.fatal("Create post fatal error");
         }
      }
   }
    ...
    * <b>Main Contributor:</b> Alex</p>
    * <p>
    * <b>SQL:</b> DELETE FROM Post WHERE authorid = ? AND topicid = ? AND text =
    * ?;</p>
    * <p>
    ...
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL:</b> INSERT INTO Person (name, username, stuId) VALUES (?,?,?);</p>
    * <p>
    * <b>How it works:</b>
    *
    *
    ...
    * @param studentId the person's student id. May be either NULL if the person
    * is not a student or a non-empty string if they are; can not be an empty
    * string.
    * @return Success if no person with this username existed yet and a new one
    * was created, failure if a person with this username already exists, fatal
    ...
    * <h1>Only designed to be used for unit tests</h1>
    * <p>
    * Deletes a Person of given username.</p>
    * <p>
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL:</b> DELETE FROM Person WHERE username = ?;</p>
    * <p>
   ...
   /**
    * <p>
    * Gets the title of the forum given the forum id</p>
    * <p>
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL:</b> SELECT title FROM Forum WHERE id = ?;</p>
    *
    * @param id the id of the forum to get.
   ...
   /**
    * <p>
    * Get the detailed view of a topic.</p>
    * <p>
    * <b>Visual Test:</b>http://localhost:8000/topic/4</p>
    * <p>
    * <b>Main Contributor:</b> Joseph</p>
    * <p>
    * <b>SQL:</b> </p>
    ...
    *
    * @param topicId the topic to get.
    * @param page if 0, fetch all posts, if n > 0, fetch posts 10*(n-1)+1 up to
    * 10*n, where the first post is number 1.
    * @return The topic view if one exists with the given id and range, (i.e.
    * for getTopic(tid, 3) there must be at least 31 posts) otherwise failure
    * (or fatal on database errors).
    */
   @Override
   ...
   /**
    * <p>
    * Set or unset a topic as favourite. Same semantics as likeTopic.</p>
    * <p>
    * <b>Visual Test:</b> </p>
    * <p>
    * <b>Main Contributor:</b> </p>
    * <p>
    * <b>SQL:</b> </p>
    * <p>
    * <b>How it works:</b>
   ...
   /**
    * <p>
    * Create a new topic in a forum.</p>
    * <p>
    * <b>Visual Test:</b> </p>
    * <p>
    * <b>Main Contributor:</b> Tom</p>
    * <p>
    * <b>SQL1:</b> INSERT INTO Topic (forumid, title) VALUES (?,?);</p>
    * <p>
    * <b>SQL2:</b> INSERT INTO Post (authorid, topicid, text) VALUES ( (SELECT
    * id FROM Person WHERE username = ?), (SELECT id FROM Topic WHERE title =
    * ?), ?);</p>
    * <p>
    * <b>How it works:</b>
    *
    ...
    * forum must exist.
    * @param username the username under which to make this post. Must refer to
    ...
    * exist, user does not exist, title or text empty); success if the post was
    * created and fatal if something else went wrong.
    */
   @Override
   public Result createTopic(long forumId, String username, String title, String text) {
      ...
      try (
         ...
         if (username.isEmpty() || title.isEmpty() || text.isEmpty()) {
            throw new RuntimeException("Cannot have empty");
         }
         ...
         if (ex.getLocalizedMessage().contains("FOREIGN KEY constraint failed")) {
            ...
            return Result.fatal("create topic failed");
         }
      } catch (RuntimeException ex) {
         try {
            c.rollback();
   ...
   /**
    * <p>
    * Like or unlike a post. Liking a post that you have already liked (or
    * unliking a post you haven't liked) is a no-op, not an error.</p>
    * <p>
    * <b>Visual Test:</b> </p>
    * <p>
    * <b>Main Contributor:</b> Alex</p>
    ...
    * @param like true to like, false to unlike.
    * @return failure if the person or post referenced to not exist, success if
    * the (un)like succeeded, fatal in case of other errors.
    */
   @Override
   ...
   /**
    * <p>
    * Utility function for checking if a topic exists</p>
    * <p>
    * <b>SQL:</b> SELECT Topic.id FROM Topic WHERE Topic.id = ?;</p>
    *
   ...
   /**
    * <p>
    * Utility function for checking if a forum exists</p>
    * <p>
    * <b>SQL:</b> SELECT id FROM Forum WHERE id = ?;</p>
    *
   ...
   /**
    * <p>
    * Utility function for checking if a person exists</p>
    * <p>
    * <b>SQL:</b> SELECT username FROM Person WHERE username = ?;</p>
    *
